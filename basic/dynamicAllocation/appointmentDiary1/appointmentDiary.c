 ...
 *  @brief 		src file for appointmentDiary API
 * 
...
struct ManagerType
{
    ...
    size_t m_numOffElement; /* Index number of meeting in that day */
...
ERRORS InsertMeeting(DayPtr* _infoPtr, Meeting* _meetingPtr)
{
    ...
    nItems = _infoPtr->m_numOffElement;
    ...
    /* Update numOffElement after before placement (and before shifting) */
    ++(_infoPtr->m_numOffElement);
    ...
    /* Place the meeting in the original index and update numOffElement*/
...
void DestroyDiary(DayPtr* _infoPtr)
{
    ...
    while( index < (_infoPtr->m_numOffElement) )
    {
        free(_infoPtr->m_buffer[index]);
        ++index;
    }
...
static ERRORS CheckRealloc(DayPtr* _infoPtr)
{
    ...
    if( _infoPtr->m_numOffElement  < _infoPtr->m_size )
    {
    	return OK;
    }
...
static void ShiftMeeting(DayPtr* _infoPtr, size_t _index, int _condition)
{
    ...
    if( 1 == _condition )
    {
        _index = _infoPtr->m_numOffElement;
    ...
    while( _index < (_infoPtr->m_numOffElement - 1 ) )
    {
         _infoPtr->m_buffer[_index] = _infoPtr->m_buffer[_index + 1];
        ++_index;
    }
